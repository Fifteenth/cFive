...
使用：onfocus="setday(this)" readonly
...
strFrame+='        <td colspan=3 align=right Author=meizz><span onclick="parent.cancelday();" style="font-size:12px;cursor: hand"';
strFrame+='         Author=meizz title="' + _VersionInfo + '"><u>取消</u></span>&nbsp;</td></tr>';
...
window.onresize=function(){
	if(outObjectName!=""&&document.all.meizzDateLayer.style.display!="none"){
		closeLayer();
		ShowCalendar(outObjectName);
	}
}
...
function closeLayer()               //这个层的关闭
  {
  	var inputName = window.document.getElementById("inputName").value;
    var _parentWin = window.dialogArguments ;
    _parentWin.document.getElementById(inputName).value = window.document.getElementById("date").value;
...
function ShowCalendar(InputBox){
	...
	if(outObjectName==""){
		outObjectName=InputBox;
	}
	
	var tt=eval("document.all."+InputBox);
	var dads  = document.all.meizzDateLayer.style;
	var th = tt;
	var ttop  = tt.offsetTop;     //TT控件的定位点高
	var thei  = tt.clientHeight;  //TT控件本身的高
