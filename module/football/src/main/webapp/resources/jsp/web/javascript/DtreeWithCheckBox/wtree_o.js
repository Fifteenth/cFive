...
dTree.prototype.getCheckedNodes=function(){
	...
	for(var n=0; n<this.aNodes.length; n++){
		...
		if(cko==null){			
				...
				if(this.aNodes[n].id == this.aNodes[j].id){
				
				alert("sucess:"+this.aNodes[j].id+":"+this.aNodes[j].name);
				alert(document.getElementById("c"+this.aNodes[j].id));
				}
...
function selectAll(root) {
	var res = new Array();
	var cko;//checkobject
	for(var n=0; n<root.aNodes.length; n++){
		//alert("c"+this.obj+n+(document.getElementById("c"+this.obj+n).checked));
		//cache the object to improve speed when you have very,very many nodes and call this function many times in one page
		//i.e. with ajax technology
		//document.getElementById("c"+this.obj+n).checked
		cko = root.co(root.aNodes[n].id);
		if(cko.checked==true){
			res[res.length]=root.aNodes[n].id;
		}
	}
	return res;
}
...
dTree.prototype.getNodeById = function (id) {
	alert("test");
	for (var n = 0; n < this.aNodes.length; n++) {
	alert(this.aNodes[n].id+":"+this.aNodes[n].name);
		if (this.aNodes[n].id == id) {
